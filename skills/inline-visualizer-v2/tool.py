...
description: Renders interactive HTML/SVG visualizations inline in chat. Includes buttons to download each visualization as self-contained HTML or as a PNG image. Requires "iframe Sandbox Allow Same Origin" to be enabled in Open WebUI Settings -> Interface. For design instructions, the model should call view_skill("visualize").
"""
...
#iv-dl-wrap{position:fixed;top:4px;right:4px;z-index:9999;display:flex;gap:4px}
#iv-dl-wrap button{width:26px;height:26px;padding:0;display:flex;align-items:center;justify-content:center;
  opacity:0.3;border-color:var(--color-border-tertiary);background:var(--color-bg-primary)}
#iv-dl-wrap button:hover{opacity:0.9;background:var(--color-bg-secondary)}
#iv-dl-wrap button[disabled]{opacity:0.15;cursor:default}
#iv-dl-wrap button svg{width:14px;height:14px;stroke:var(--color-text-secondary);fill:none;
...
function toast(msg, kind) {
  ...
  if (!wrap) {
    ...
    wrap.style.cssText =
      'position:fixed;top:4px;right:64px;z-index:9998;' +
...
// PNG download label — every _ivStr entry contains the literal token
// "HTML", so the PNG variant is just that token swapped. Keeps the two
// labels in lockstep across all locales without a second hand-written table.
var _ivPngStr = {};
for (var _ivPngK in _ivStr) {
  _ivPngStr[_ivPngK] = _ivStr[_ivPngK].replace('HTML', 'PNG');
}
...
(function() {
  ...
  var btnPng = document.getElementById('iv-dl-png-btn');
  if (btnPng) btnPng.title = _ivPngStr[_ivLang] || _ivPngStr.en;
...
// ---------------------------------------------------------------------------
// Download as self-contained HTML or as a PNG image
// ---------------------------------------------------------------------------
...
// Filesystem-safe base name from the document title (no extension).
function _ivFileBase() {
...
// Hand a Blob to the browser as a download. Shared by the HTML and PNG
// exporters so the iOS / desktop quirks live in exactly one place.
...
function _ivDownload() {
  ...
  _ivSaveBlob(blob, _ivFileBase() + '.html');
}
...
// other locale falls back to English (PNG generation is rarely slow enough
// for the wait toast to register, and the label is already in the tooltip).
var _ivPngWaitStr = {
  en: 'Preparing PNG…', de: 'PNG wird vorbereitet…',
  cs: 'Příprava PNG…', hu: 'PNG előkészítése…',
  hr: 'Priprema PNG-a…', pl: 'Przygotowywanie PNG…',
  fr: 'Préparation du PNG…', nl: 'PNG voorbereiden…'
};
var _ivPngErrStr = {
  en: 'PNG export failed', de: 'PNG-Export fehlgeschlagen',
  cs: 'Export PNG selhal', hu: 'A PNG exportálása nem sikerült',
...
function _ivDownloadPng() {
  var btn = document.getElementById('iv-dl-png-btn');
  if (btn && btn.disabled) return;          // ignore re-entrant clicks
  if (btn) btn.disabled = true;
  try { toast(_ivPngWaitStr[_ivLang] || _ivPngWaitStr.en, 'info'); } catch(e) {}
...
DOWNLOAD_BUTTON = (
    '<div id="iv-dl-wrap">'
    '<button id="iv-dl-btn" onclick="_ivDownload()" title="Download as HTML">'
    '<svg viewBox="0 0 16 16"><path d="M8 2v8M5 7l3 3 3-3"/><path d="M3 12h10"/></svg>'
    '</button>'
    '<button id="iv-dl-png-btn" onclick="_ivDownloadPng()" title="Download as PNG">'
    '<svg viewBox="0 0 16 16"><rect x="2.5" y="3.5" width="11" height="9" rx="1.5"/>'
    '<circle cx="6" cy="6.5" r="1"/><path d="M3 11l3-3 2 2 2.5-2.5L13 11"/></svg>'
    '</button>'
    '</div>'
)
